...
public class YahooFinanceAPI {
	...
	public JSONArray getCandleData(String symbol, String interval, String range, Date fromDate, Date toDate) {
		...
		try {
			...
			if (responseCode == HttpURLConnection.HTTP_OK) {
				try(BufferedReader in = new BufferedReader(new InputStreamReader(con.getInputStream()));){
					...
					for(int i = 0; i < timestampArray.length(); i++) {
						...
						newObj.put("open", openArray.getBigDecimal(i).setScale(2, RoundingMode.HALF_EVEN));
						newObj.put("high", highArray.getBigDecimal(i).setScale(2, RoundingMode.HALF_EVEN));
						newObj.put("low", lowArray.getBigDecimal(i).setScale(2, RoundingMode.HALF_EVEN));
						newObj.put("close", closeArray.getBigDecimal(i).setScale(2, RoundingMode.HALF_EVEN));
						newObj.put("volume", volumeArray.get(i));
			...
			System.out.println("YahooFinanceAPIException: "+ e);
		}
		return jsonArray;
	}
}
